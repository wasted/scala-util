...
class ScheduleSpec extends Specification {
  ...
  "Schedule should have done 3 tests where results" should {
    "be true for Schedule.once" in { result1 must_== true }
    "be true for Schedule.again (first run)" in { result2 must_== true }
    "be true for Schedule.again (second run)" in { result3 must_== true }
  }
